...
def update_map(arena, bot):
    for t, _b in enumerate(bot.path):
        _len = len(bot.path)-1
        arena.space_time.append({"coordinate": _b["coordinate"], "time": (t-_len), "value": bot.id})
        if t == _len:
            continue
        arena.space_time.append({"coordinate": _b["coordinate"], "time": (t-_len) + 1, "value": bot.id})
...
def show_arena(arena):  # show maps in normal x and y
    with open('reservationTableReverse.txt', 'w') as _txt:  # import map from txt
...
def check(bot, _xt, _yt, _maps, _t):
    if any(_map.get("coordinate", None) == (_xt, _yt) and _map.get("time", None) == _t for _map in
           _maps.space_time):  # check hit obstacle or other bot
        return False
    else:
        return True
...
def calc(bot, xd, yd, _t):
    ...
    hx = abs(xd - bot.start[0]) + abs(yd - bot.start[1])
    fx = hx + bot.open_list[-1]["gx"]
    bot.open_list[-1].update({"hx": hx, "fx": fx, "cost": fx})


def plan(bot, arena, maximum_try):
    xc, yc = bot.finish  # x and y finish as first coordinate
    try_counter = 0
    time = -1  # time is 1 at first movement from finish, in real life should change it
    ...
    while xc != bot.start[0] or yc != bot.start[1]:  # until find the start
...
def path(bot):
    ...
    while bot.path[-1]["coordinate"] != bot.finish:
        for _close in bot.closed_list:
            if _close["coordinate"] == bot.path[-1]["parent"] and _close["time"] == _t:
                bot.path.append({"coordinate": _close["coordinate"], "parent": _close["parent"]})
                _t += 1
